...
Needs rasterio, rasterstats, lerc - none of which are part of the site's
own toolchain. Run `./tools/setup.sh --geo` once to provision a venv with
all of them, then use ITS python3 to run this script:
    ~/.local/govapiportal-geo-venv/bin/python3 tools/heat_build.py ...

The per-city raster "stain" overlay for canopy-map.html (formerly this
file's own "blobs" command) now lives in tools/heat_blobs.py instead - it
needs the same city/neighborhood boundary geometry this file's own
build_neighborhoods()/build_streets() get from canopy_build.py, which
hard-imports `from osgeo import ogr` at file scope. osgeo isn't installed
here (no GDAL Python bindings, no network to add them), so that whole
import chain - and this file's "blobs" command with it - never actually
ran in this environment; heat_blobs.py reads the same boundary geometry a
different way (already-computed ITM-meter rings in src/map-boundaries-
*.js, via blob_geo.py) and needs no osgeo at all.
