...
def write_js(data):
    ...
    out_path.write_text(
        f"// Per-city high-resolution tree-canopy raster overlay (real crown polygons, not per-neighborhood %), computed {stamp}\n"
        f"// Generated by tools/canopy_blobs.py - do not edit by hand.\n"
        f"export const CANOPY_BLOBS = {json.dumps(data, ensure_ascii=False, separators=(',', ':'))};\n",
        encoding="utf-8",
    )
