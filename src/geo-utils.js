...
async function stitchBasemap(cornersWgs84, size, kind = 'street') {
  ...
  const mys = cornersMerc.map((c) => c[1]);
  const mercWidth = Math.max(...mxs) - Math.min(...mxs);
  const mercHeight = Math.max(...mys) - Math.min(...mys);
  // Zoom picked off whichever real-world span (width OR height) is LARGER,
  // so that one maps to `size` px - the other comes out proportionally
  // smaller, not stretched to match. Every existing caller (blue-lines.js/
  // area-cleanup.html/trip-report.html, via bboxAround's always-square
  // radius box) has mercWidth == mercHeight already, so this is a no-op for
  // them. canopy-map.js's country-level basemap does NOT have a square
  // bbox (Israel's full extent is ~3:1 tall vs wide) - picking zoom off
  // width alone (the old behavior) there, then force-stretching the result
  // into a square output below, visibly distorted the map enough to read
  // as fake 3D/tilted perspective (real user report, confirmed by eye
  // against this exact math, not assumed).
  let zoom = Math.round(Math.log2((2 * Math.PI * MERCATOR_R) / (TILE_PX * Math.max(mercWidth, mercHeight) / size)));
  ...
  // Natural pixel size at the zoom already picked above - NOT stretched to
  // a fixed size x size box (that stretch is exactly what used to distort
  // a non-square bbox, see this function's own comment above). pxMin/pyMin
  // etc. are already real pixel coordinates at `zoom`, so a straight 1:1
  // copy (no destination scaling) is both correct and simpler than the
  // old to-size rescale.
  const outW = Math.round(pxMax - pxMin);
  const outH = Math.round(pyMax - pyMin);
  ...
  octx.drawImage(
    canvas,
    pxMin - txMin * TILE_PX, pyMin - tyMin * TILE_PX, outW, outH,
    0, 0, outW, outH,
  );
  ...
  octx.fillRect(0, outH - 16, tw + 8, 16);
  octx.fillStyle = '#000';
  octx.fillText(label, 4, outH - 4);

  const project = (lon, lat) => {
    const [px, py] = mercatorToPixel(...lonLatToMercator(lon, lat), zoom);
    return [px - pxMin, py - pyMin];
  };
