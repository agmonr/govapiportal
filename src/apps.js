...
const CATEGORY_ORDER = ['community', 'civic', 'home', 'trees', 'money', 'direct', 'external'];

// accidents.html used to be pulled out here and rendered standalone with no
// category heading, as the page's own headline. That stopped making sense
// once it moved into the same "חברה וקהילה" category as mortality-compare.html
// (see apis.json) - by explicit follow-up request, the two now render
// together, grouped, at the top of the page instead. No app is pinned
// anymore; `pinned` below is always undefined and every app renders through
// the normal per-category path.
const PINNED_APP_ID = null;
