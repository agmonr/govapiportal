...
async function run(address, radius, showLabels, showPoints, showMetroZone) {
  ...
  const [landUse, planBoundaries, points, basemap, metro, planLinks, metroZoneRings] = await Promise.all([
    // Fetched separately from the plan boundaries below, and drawn at
    // reduced opacity - land use (even with the truly-meaningless 995/996
    // catch-alls excluded) still includes real, large area-wide
    // designations, e.g. a heritage conservation zone spanning several km2
    // over central Tel Aviv. That's genuine information, not a bug to
    // filter out - but at full strength it reads as foreground noise
    // burying the basemap and the plan boundaries underneath it, so it's
    // pushed toward the background instead: still visible, not dominant.
    fetchXplanImage(bbox, IMAGE_SIZE, [4], showLabels),
...
function start() {
  ...
  // A linked search (?address=...&radius=...&labels=1&points=0&metroZone=0)
