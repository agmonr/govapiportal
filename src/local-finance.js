...
/* ---------- sessionStorage-backed fetch cache + status bar ----------
 * Every live data.gov.il call on this page (national summary, liabilities,
 * per-authority bundles, CBS population, jurisdiction area, statement rows,
 * roster, new-year discovery) is routed through cachedCall() below, keyed
 * by exactly what's being asked for - a repeat visit/reload within the same
 * TAB SESSION (sessionStorage, not localStorage - clears on tab close,
 * unlike a permanent offline cache this data's own "כמה שגוי יכול המידע
 * להיות" disclaimer would rather not imply) reuses the cached answer
 * instead of re-fetching. `#finDataStatus` (see style.css's own
 * .fin-data-status) surfaces this live: visible while anything is in
 * flight, then a brief "X מהמטמון, Y מהרשת" summary once a burst of calls
 * settles (debounced, so a Promise.all of several calls collapses into one
 * final message instead of flickering per-call), auto-hiding itself after.
 * A quota-exceeded/unavailable sessionStorage (private browsing, storage
 * full) degrades to "no caching" silently - never blocks a real fetch. */

let cacheInFlight = 0;
let cacheBatchActive = false;
let cacheBatchHits = 0;
let cacheBatchMisses = 0;
let statusHideTimer = null;

function dataStatusEl() { return el('finDataStatus'); }

function showLoadingStatus() {
  const box = dataStatusEl();
  if (!box) return;
  clearTimeout(statusHideTimer);
  box.hidden = false;
  box.classList.add('fin-status-loading');
  box.textContent = `טוען נתונים מ-data.gov.il… (${num(cacheBatchHits)} מהמטמון, ${num(cacheBatchMisses)} מהרשת${cacheInFlight ? `, ${num(cacheInFlight)} בקשות פעילות` : ''})`;
}

// Debounced, not immediate - a burst of calls (e.g. fetchAuthorityBundles'
// own 9-years-at-once Promise.all) settles one at a time; without this the
// message would flip to "done" after the FIRST of the nine finishes, then
// flicker straight back to "loading" for the next eight. Re-checks
// cacheInFlight when it actually runs (not when it was scheduled), so it
// correctly bails and lets the next settle's own debounced call take over
// if something new started in the meantime.
const showDoneStatusDebounced = debounce(() => {
  if (cacheInFlight > 0) return;
  const box = dataStatusEl();
  if (!box) return;
  box.classList.remove('fin-status-loading');
  box.textContent = `הנתונים נטענו — ${num(cacheBatchHits)} מהמטמון, ${num(cacheBatchMisses)} מהרשת.`;
  cacheBatchActive = false;
  clearTimeout(statusHideTimer);
  statusHideTimer = setTimeout(() => { box.hidden = true; }, 4000);
}, 200);

const cacheKey = (...parts) => `lf:${parts.map((p) => (typeof p === 'string' ? p : JSON.stringify(p))).join(':')}`;

/** `key` identifies WHAT is being asked for (see the named wrappers below -
 *  cachedDsFilter/cachedPopulation/etc.), `fn` is only called on a cache
 *  miss. A fresh batch's hit/miss counters reset the first time this is
 *  called after being fully idle, so the eventual "done" summary reflects
 *  the action that was JUST taken, not an ever-growing session total. */
async function cachedCall(key, fn) {
  if (!cacheBatchActive) { cacheBatchActive = true; cacheBatchHits = 0; cacheBatchMisses = 0; }
  let cached;
  try { cached = sessionStorage.getItem(key); } catch { cached = null; }
  if (cached != null) {
    cacheBatchHits += 1;
    showLoadingStatus();
    showDoneStatusDebounced();
  ...
  try {
    ...
    cacheBatchMisses += 1;
    try { sessionStorage.setItem(key, JSON.stringify(result)); } catch { /* quota exceeded/unavailable - caching is best-effort, never blocks the real result */ }
    return result;
  } finally {
    cacheInFlight -= 1;
    showDoneStatusDebounced();
...
const dsQueryKey = (resourceId, params) => cacheKey('ds', resourceId, params);
function dsFilterParams(filters, limit = 10000) {
  const params = { limit: String(limit) };
  if (filters) params.filters = JSON.stringify(filters);
  return params;
}
/** Whether a dsFilter-shaped call is already cached, WITHOUT making it -
 *  downloadAllYearsForAuthority() below uses this to skip its own politeness
 *  pacing between years for a year already sitting in the cache: there's no
 *  live request in flight to be polite to for those years. */
function isDsFilterCached(resourceId, filters, limit = 10000) {
  try { return sessionStorage.getItem(dsQueryKey(resourceId, dsFilterParams(filters, limit))) != null; } catch { return false; }
}

const cachedDsQuery = (resourceId, params) => cachedCall(dsQueryKey(resourceId, params), () => dsQuery(resourceId, params));
/** Same shape as datastore.js's own dsFilter (JSON-stringifies `filters`
 *  once here rather than at every call site), just cached. */
function cachedDsFilter(resourceId, filters, limit = 10000) {
  return cachedDsQuery(resourceId, dsFilterParams(filters, limit));
}
const cachedPopulation = (name) => cachedCall(cacheKey('pop', name), () => fetchPopulation(name));
const cachedPopulations = () => cachedCall(cacheKey('pops'), () => fetchPopulations());
// discoverNewYears' own package_show lookups - a plain fetch, not dsQuery-
// shaped, so its own thin wrapper rather than forcing it through
// cachedDsQuery's resource/params shape.
const cachedPackageShow = (pkg) => cachedCall(cacheKey('pkg', pkg), async () => {
...
async function loadRoster() {
  try {
    ...
    const { records } = await cachedDsFilter(cfg.resourceId, {
...
async function discoverNewYears() {
  ...
  for (const pkg of FINANCE_PACKAGES) {
    try {
      const j = await cachedPackageShow(pkg);
...
async function renderCharts(summary) {
  ...
  if (liabilities && liabilities !== 'unsupported') {
    ...
    // cities entirely.
    const populations = await cachedPopulations();
...
async function fetchAuthorityBundles(authorities) {
  const perYear = await Promise.all(YEARS_DESC.map(async (year) => {
    ...
    try {
      const { records } = await cachedDsFilter(cfg.resourceId, filters);
...
async function renderAuthorityLiabRank() {
  ...
  const [summary, liabilities, populations] = await Promise.all([
    fetchSummary(state.year), fetchLiabilities(state.year), cachedPopulations(),
  ]);
...
async function renderAuthorityCharts() {
  ...
  const [
    ...
    cachedPopulation(state.authority),
    Promise.all(compareNames.map((name) => cachedPopulation(name))),
...
async function fetchJurisdictionArea(authority) {
  ...
  try {
    const { records } = await cachedDsFilter(cfg.resourceId, { שם_רשות: authority, [cfg.sheetField]: JURISDICTION_SHEET, שורה: JURISDICTION_ROW });
...
async function renderStatement() {
  ...
  try {
    ...
    if (cfg.yearFilter) filters[cfg.yearFilter.field] = cfg.yearFilter.value;
    const { records, total } = await cachedDsFilter(cfg.resourceId, filters);
...
async function downloadAllYearsForAuthority() {
  ...
  try {
    ...
    for (const year of YEARS_DESC) {
      ...
      const filters = { שם_רשות: state.authority };
      if (cfg.yearFilter) filters[cfg.yearFilter.field] = cfg.yearFilter.value;
      const alreadyCached = isDsFilterCached(cfg.resourceId, filters);
      try {
        const { records } = await cachedDsFilter(cfg.resourceId, filters);
        if (records.length) {
          found += 1;
      ...
      // Politeness pacing (same spirit as elsewhere on this site) only
      // matters for a real outbound request - a year already sitting in the
      // cache never left the browser, so pacing it too would just slow the
      // download down for no reason.
      if (!alreadyCached) await new Promise((r) => { setTimeout(r, 200); });
    }
    if (!all.length) {
